...
"""
_Watch is ported from from google.cloud.firestore_v1.watch
...
class WatchDocTree(object):
    # TODO: this class has been modified to suit the purpose of flask-boiler
    # TODO: note that the RAM usage will keep increasing since consumed
    #           document snapshot in _removed are not deleted
    #           so that we can find the original version of the document
    ...
    def keys(self):
        return [*self._removed.keys(), *self._dict.keys()]

    def _copy(self):
        wdt = WatchDocTree()
        wdt._dict = self._dict.copy()
        wdt._index = self._index
        wdt._removed = self._removed.copy()
        self = wdt
        return self
    ...
    def remove(self, key):
        self = self._copy()
        self._removed[key] = self._dict[key]
        del self._dict[key]
        return self
...
class _Watch(object):
    ...
    def __init__(
        ...
        # TODO: recover this line somewhere
        ...
        self.change_log = defaultdict(list)
    ...
    #     if self.resume_token is not None:
    #         self._targets["resume_token"] = self.resume_token
    #     if INITIAL_TARGET_ID in self._targets:
    ...
    def add_target(self, target, callback):
        ...
        if target_id in self._targets:
            raise ValueError
        self._targets[target_id] = target
        self._target_callbacks[target_id] = callback
        self._rpc.send(
            firestore_pb2.ListenRequest(
                database=self._firestore._database_string,
                add_target=self._targets[target_id]
            )
        )
    ...
    def for_query(
        ...
        return cls(
            query,
            query._client,
            {"query": query_target, "target_id": INITIAL_TARGET_ID},
            query._comparator,
            snapshot_callback,
            snapshot_class_instance,
            reference_class_instance,
    ...
    def _on_snapshot_target_change_no_change(self, proto):
        ...
        # TODO: move resume token key in self._targets to elsewhere
        target_ids = list(self._targets.keys()) if no_target_ids \
            else change.target_ids
    ...
    def push(self, read_time, next_resume_token):
        ...
        """
        # deletes, adds, updates = _Watch._extract_changes(
    ...
    def _current_size(self):
        ...
        """
        deletes, adds, _ = _Watch._extract_changes(self.doc_map, self.change_map, None)
